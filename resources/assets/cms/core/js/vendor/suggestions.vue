<template>
    <div style="position:relative" v-bind:class="{'open':openSuggestion, 'md-load': true}">
        <input class="form-control" type="text" :value="value" @input="updateValue($event.target.value)"
          ...
          @keydown.up = 'up'
        >
         <label v-show="loading" class="pull-right"><i class="load fa fa-spinner fa-spin"></i></label>
        <ul class="dropdown-menu" style="width:100%">
            <li v-for="(suggestion, index) in matches"
                v-bind:class="{'active': isActive(index)}"
                @click="suggestionClick(index)"
            >
              <a href="javascript:void(0)">{{ suggestion.name }} <small>{{ suggestion.email }}</small>
              </a>
            </li>
        </ul>
    </div>
  ...
  computed: {
    ...
    matches () {
      return this.suggestions.filter((obj) => {
        return obj.name.toLowerCase().indexOf(this.value) >= 0
      })
    },
