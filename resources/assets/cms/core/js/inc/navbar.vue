		...
		<nav class="navbar navbar-static-top">
			...
			<div class="navbar-custom-menu">
				<ul class="nav navbar-nav">
					<router-link to="/messages" tag="li" class="dropdown messages-menu">
						<a>
						<i class="fa fa-envelope-o"></i>
						<span class="label label-success">{{ newMsgCount }}</span>
					    </a>
					</router-link>
					<router-link to="/profile" class="dropdown user user-menu" tag="li">
						<a><img v-bind:src="userImage" alt="" class="user-image">
						<span class="hidden-xs">{{ currentUser.name }}</span></a>
					</router-link>
					<router-link v-if="currentUser.is_admin == 1" tag="li" to="/settings"><a><i class="fa fa-gears"></i></a></router-link>
				</ul>
			</div>
		</nav>
	</header>
